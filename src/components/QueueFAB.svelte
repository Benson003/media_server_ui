...
{#if open}
	<div class="fixed bottom-24 right-6 bg-gray-900 text-white p-4 rounded-xl shadow-xl w-72 z-40 max-h-96 overflow-y-auto space-y-3">
		{#if list.length === 0}
			<p class="text-sm text-gray-400 text-center">Queue is empty</p>
		{:else}
			{#each list as item}
				<div class="flex justify-between items-center bg-gray-800 px-4 py-2 rounded-lg shadow-inner">
					<span class="truncate w-48">{item.name}</span>
					<button
...
>
	{#if open}
		<div class="relative w-5 h-5">
			<span class="absolute top-1/2 left-0 w-5 h-1 bg-white rounded-2xl transform -translate-y-1/2 rotate-45"></span>
			<span class="absolute top-1/2 left-0 w-5 h-1 bg-white rounded-2xl transform -translate-y-1/2 -rotate-45"></span>
		</div>
	{:else}
